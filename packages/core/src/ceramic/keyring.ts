...
function deriveKeySet(seed: Uint8Array, v03ID?: string): FullKeySet {
  ...
  const encryption = generateKeyPairFromSeed(
...
export default class Keyring {
  ...
  constructor(seed?: Uint8Array, v03ID?: string) {
    ...
    let encKid = encodeKey(
      this._keySets[LATEST].publicKeys.encryption,
      'x25519'
    ).slice(-15)
  ...
  async loadPastSeeds(pastSeeds: Array<JWE>): Promise<void> {
    ...
    while (jwe) {
      ...
      this._keySets[version] = deriveKeySet(
        new Uint8Array(decrypted[version]),
        this._v03ID
      )
  ...
  getAsymDecrypter(fragments: Array<string> = [], version?: string): Decrypter {
    if (!version) {
      const fragmentWithKey = fragments.find(
        (fragment: string) => this._versionMap[fragment]
      )
  ...
  async asymDecryptJWE(
    ...
    return decodeCleartext(
      await decryptJWE(jwe, this.getAsymDecrypter(kids, version))
    )
  }

  async asymEncryptJWE(
  ...
  get3idState(genesis?: boolean): ThreeIdState {
    ...
    const state: ThreeIdState = {
      metadata: {
        controllers: [`did:key:${encodeKey(keys.management, 'secp256k1')}`],
      },
      content: {
        publicKeys: {
