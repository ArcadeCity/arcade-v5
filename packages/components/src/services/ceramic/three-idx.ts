...
export class ThreeIDX {
  ...
  async createAuthLinkUpdate({
    did,
  }: NewAuthEntry): Promise<AuthLinkDocUpdate> {
  ...
  async applyAuthLinkUpdate({
    docid,
    commit,
    did,
  }: AuthLinkDocUpdate): Promise<void> {
    // @ts-ignore
  ...
  async createIDX(newEntry?: NewAuthEntry): Promise<void> {
    const docUpdatePromise = newEntry
      ? this.createAuthLinkUpdate(newEntry)
  ...
  async addAuthEntries(newEntries: Array<NewAuthEntry>): Promise<void> {
    const linkDocUpdatesPromise = Promise.all(
      newEntries.map(this.createAuthLinkUpdate.bind(this))
    )
    const { authMap, pastSeeds } = this.docs[KEYCHAIN_DEF].content
  ...
  async addKeychainToIDX(): Promise<void> {
    ...
    if (!content || !content[KEYCHAIN_DEF]) {
      ...
      const nextMetadata = this.docs.idx.metadata.schema
        ? undefined
